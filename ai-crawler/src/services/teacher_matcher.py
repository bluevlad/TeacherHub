...
class TeacherMatcher:
    ...
    def _build_pattern(self, name: str) -> str:
        ...
        # 한글 이름의 경우: 앞뒤로 한글/영문/숫자가 아닌 경우 매칭
        # 예: "이선재" -> "이선재 강사", "이선재쌤" 등에서 매칭
        # 하지만 "이선재국어" 같은 경우도 매칭되어야 함

        # 간단한 접근: 이름 앞뒤에 공백, 특수문자, 문장 시작/끝이 있으면 매칭
        # 또는 뒤에 '쌤', '강사', '선생님' 등이 오면 매칭

        # 좀 더 유연한 패턴: 이름만 찾되, 단독으로 사용되는 경우
        # (?:^|[^가-힣a-zA-Z0-9]) + name + (?:[^가-힣a-zA-Z0-9]|$)
        pattern = f'(?:^|[^가-힣a-zA-Z0-9]){escaped}(?:[^가-힣a-zA-Z0-9]|$)'
